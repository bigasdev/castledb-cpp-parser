...
int main() {
    ...
    CastleDBConverter converter("helloworld.json");
    ...
    while(working){
        if(!converted){
            for (auto& m_data : data_db) {
                ...
                for(size_t i = 0; i < std::stoi(m_data.data["X"]); i++){
                        
                    for(size_t j = 0; j < std::stoi(m_data.data["Y"]); j++){
        ...
        if(converter.update("helloworld.json")){
            system("cls");
